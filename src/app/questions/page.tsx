"use client"
import React, { useEffect, useState } from 'react'
import DashboardLayout from '../components/DashboardLayout'
import { FaHouse } from 'react-icons/fa6'
import Link from 'next/link'
import { Question } from '../components/RecentQuestionsTable'
import TableComponent from '../components/Table'
import { db, auth } from "@/firebase/firebase"
import { collection, getDocs, query, where } from "firebase/firestore"
import { usePermissions } from '../hooks/usePermissions'
import { UserRole } from '../services/permissions'

const Questions = () => {
  const { userRole, loading } = usePermissions();
  const [questions, setQuestions] = useState<Question[]>([]);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchQuestions = async () => {
      if (!userRole) return;

      try {
        const isAdminOrPresident = userRole === UserRole.ADMIN || userRole === UserRole.PRESIDENT;
        let questionsQuery;

        if (isAdminOrPresident) {
          questionsQuery = collection(db, "Questions");
        } else {
          const currentUser = auth.currentUser;
          questionsQuery = query(
            collection(db, "Questions"),
            where("userId", "==", currentUser?.uid)
          );
        }

        const querySnapshot = await getDocs(questionsQuery);
        const questionData = querySnapshot.docs.map(doc => {
          const data = doc.data();

          // Convert Firebase timestamp to formatted date string
          const date = new Date(data.uploadDate);
          const formattedDate = date.toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric'
          });

          return {
            id: doc.id,
            course: data.course,
            lecturer: data.lecturer,
            department: data.department,
            questionsUrl: data.questionsUrl,
            uploadDate: formattedDate,
          };
        });
        setQuestions(questionData);
      } catch (error: Error | any) {
        setError(error.message);
      }
    };

    fetchQuestions();
  }, [userRole]);

  const tableHeader = [
    { column: "course", header: "Course" },
    { column: "lecturer", header: "Lecturer" },
    { column: "department", header: "Department" },
    { column: "questionsUrl", header: "Questions" },
    { column: "uploadDate", header: "Upload Date" }
  ];

  return (
    <div>
      <DashboardLayout active="questions">
        <div className="py-8 lg:px-8 px-5 lg:min-h-[91.8vh] min-h-[94vh] size-full bg-gray-50">
          <div className="flex gap-5 justify-between items-center">
            <div className="flex flex-col gap-1">
              <h1 className="text-3xl font-bold text-teal-900">Questions</h1>
              <div className="flex gap-2 items-center text-sm">
                <Link href="/dashboard" className='pb-1'><FaHouse color='#9ca3af' /></Link> /
                <span className="text-gray-600 font-semibold">questions</span>
              </div>
            </div>
            <a href="/questions/add">
              <button className="flex gap-2 items-center bg-teal-600 hover:bg-teal-500 active:bg-teal-700 text-white px-8 py-2 rounded-md font-semibold hover:scale-105 transition duration-300">
                Add Question
              </button>
            </a>
          </div>

          <div className="w-full mt-10">
            <TableComponent
              tableHeader={tableHeader}
              tableData={questions}
              loading={loading}
              error={error}
              title='Question'
              fileName='questions'
            />
          </div>
        </div>
      </DashboardLayout>
    </div>
  );
};

export default Questions;